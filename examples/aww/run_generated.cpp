...
static void prepare_input(float x[1][1][49][10], float scale, int zero_point)
{
#ifdef INPUT_IS_PREQUANTIZED
	(void)scale;
	(void)zero_point;
#else
	dequantize_inplace(x, scale, zero_point);
#endif
}
                                         ...
                                         int input_zero_point)
{
	...
	prepare_input(x, input_scale, input_zero_point);
...
static RunStats run_dataset(const std::vector<SampleRef>& samples, float input_scale, int input_zero_point)
{
	...
	for (std::size_t i = 0; i < samples.size(); i++) {
		...
		s.tampering_detected = s.tampering_detected || detected;
		s.tampering_detections += TAMPERING_DETECTIONS;
		s.fault_injected = s.fault_injected || FAULT_INJECTED;
...
int main(int argc, char** argv)
{
	...
	if (sweep) {
		...
		const uint32_t effective_sweep_seed = sweep_seed_set ? sweep_seed : rd();
		std::mt19937 rng(effective_sweep_seed);
		double baseline_accuracy = 0.0;
		double baseline_accuracy_corrected = 0.0;
		...
		// Baseline (fault-free) accuracy for this binary/config (useful for plotting).
		{
			...
			FAULT_ENABLED = prev_fault_enabled;
			baseline_accuracy = st0.samples ? (double)st0.correct / (double)st0.samples : 0.0;
			baseline_accuracy_corrected =
			    st0.samples ? (double)st0.correct_corrected / (double)st0.samples : 0.0;
			std::cout << "baseline_accuracy=" << fmt4(baseline_accuracy)
			          << " baseline_accuracy_corrected=" << fmt4(baseline_accuracy_corrected)
			          << "\n";
		}

		std::cout << "sweep_layers=" << SWEEP_LAYER_COUNT << " sweep_patterns=" << patterns.size()
		          << " sweep_indexes=" << sweep_indexes << " sweep_values=" << values.size()
		          << " sweep_seed=" << effective_sweep_seed << "\n";
			...
			for (const auto& pat : patterns) {
				...
				for (float val : values) {
					...
					for (std::size_t t = 0; t < sweep_indexes; t++) {
						...
						const double detected_sample_rate =
						    st.samples ? (double)st.detected_samples / (double)st.samples : 0.0;
						std::cout << "trial=" << t << " order=" << li << " layer=" << layer_id
						          << " op=" << layer_op << " pattern=" << pat << " value=" << val
						          << " fault_index=" << FAULT_INDEX << " samples=" << st.samples
						          << " accuracy=" << fmt4(acc)
						          << " accuracy_drop=" << fmt4(baseline_accuracy - acc)
						          << " accuracy_corrected=" << fmt4(acc_corr)
						          << " accuracy_corrected_drop=" << fmt4(baseline_accuracy_corrected - acc_corr)
						          << " detected_samples=" << st.detected_samples
						          << " detected_sample_rate=" << fmt4(detected_sample_rate)
						          << " tampering_detections=" << st.tampering_detections
						          << " fault_injections=" << st.fault_injections << "\n";
					}
					...
					std::cout << "order=" << li << " layer=" << layer_id << " op=" << layer_op << " pattern=" << pat
					          << " value=" << val
	...
	RunStats st;
	{
		...
		st = run_dataset(samples, input_scale, input_zero_point);
		const double acc_corr =
		    st.samples ? (double)st.correct_corrected / (double)st.samples : 0.0;
		std::cout << "accuracy_corrected=" << fmt4(acc_corr) << " detected_samples=" << st.detected_samples
		          << "\n";
	}
	std::cout << "tampering_detected=" << (st.tampering_detected ? "true" : "false")
	          << " tampering_detections=" << st.tampering_detections << "\n";
	std::cout << "fault_injected=" << (st.fault_injected ? "true" : "false")
	          << " fault_injections=" << st.fault_injections << "\n";
	return 0;
}

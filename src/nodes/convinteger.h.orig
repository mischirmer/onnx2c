...
class ConvInteger : public SpatialFilter {
	...
	void print_im2col_matmul(std::ostream& dst) const
	{
		...
		if (checksum_enabled && randomized_enabled) {
			INDT_2 << "/* Precompute randomized weighted B-reductions once per tile/check */" << std::endl;
		...
		if (options.abyzft_gemm) {
			INDT_5 << "int32_t abyzft_scaleB_m = abyzft_scaleB[m - m0];" << std::endl;
			INDT_5 << "int64_t abyzft_scaleAB = (int64_t)abyzft_scaleA * (int64_t)abyzft_scaleB_m;" << std::endl;
			INDT_5 << "int64_t abyzft_scaleAB_mag = (abyzft_scaleAB < 0) ? -abyzft_scaleAB : abyzft_scaleAB;" << std::endl;
			INDT_5 << "int32_t abyzft_scaleAB_sign = (abyzft_scaleAB < 0) ? -1 : 1;" << std::endl;
			INDT_5 << "uint32_t abyzft_shiftAB = ABYZFT_pow2_shift_u64((uint64_t)abyzft_scaleAB_mag);" << std::endl;
			INDT_5 << "int64_t acc_scaled = 0;" << std::endl;
			if (get_number_of_inputs() < 9)
				INDT_5 << "int32_t acc32 = 0;" << std::endl;
			else
				INDT_5 << "int32_t acc32 = bias[m];" << std::endl;
			INDT_5 << "const int16_t* w_scaled = w_scaled_tile[m - m0];" << std::endl;
			INDT_5 << "for( uint32_t kk2=0; kk2<K; kk2++ ) acc_scaled += (int64_t)col_scaled[kk2] * (int64_t)w_scaled[kk2];" << std::endl;
			if (get_number_of_inputs() < 9)
				INDT_5 << "acc32 = (abyzft_scaleAB_mag != 0) ? (int32_t)(abyzft_scaleAB_sign * ABYZFT_descale_pow2_i64(acc_scaled, abyzft_shiftAB)) : 0;" << std::endl;
			else
				INDT_5 << "acc32 += (abyzft_scaleAB_mag != 0) ? (int32_t)(abyzft_scaleAB_sign * ABYZFT_descale_pow2_i64(acc_scaled, abyzft_shiftAB)) : 0;" << std::endl;
		}
		else {
		...
		if (options.abyzft_gemm)
			INDT_5 << "int64_t scaled_fault_delta = (int64_t)fault_delta;" << std::endl;
		INDT_5 << "if( FAULT_MODEL==0 ) {" << std::endl;
		if (options.abyzft_gemm)
			INDT_5 << "if( out_idx == FAULT_INDEX ) { acc_scaled += scaled_fault_delta; acc32 = (abyzft_scaleAB_mag != 0) ? (int32_t)(abyzft_scaleAB_sign * ABYZFT_descale_pow2_i64(acc_scaled, abyzft_shiftAB)) : acc32; FAULT_INJECTED = true; FAULT_INJECTIONS++; }" << std::endl;
		else
		...
		if (options.abyzft_gemm)
			INDT_5 << "if( delta != 0 ) { acc_scaled += (int64_t)delta; acc32 = (abyzft_scaleAB_mag != 0) ? (int32_t)(abyzft_scaleAB_sign * ABYZFT_descale_pow2_i64(acc_scaled, abyzft_shiftAB)) : acc32; FAULT_INJECTED = true; FAULT_INJECTIONS++; }" << std::endl;
		else
		...
		if (checksum_enabled) {
			if (randomized_enabled) {
				...
				if (randomized_checks == 1) {
					...
					INDT_6 << "}" << std::endl;
					INDT_5 << "double pred = 0.0;" << std::endl;
					INDT_5 << "for( uint32_t kk2=0; kk2<K; kk2++ ) pred += (double)col[kk2] * randomized_brs_cache[chk][tile_idx][kk2];" << std::endl;
					if (gvfa_enabled) {
						INDT_5 << "if( fabs(randomized_sumC - pred) > (double)" << options.abft_eps << " ) { TAMPERING_DETECTED = true; TAMPERING_DETECTIONS++; break; }" << std::endl;
					}
					else {
						INDT_5 << "if( randomized_sumC != pred ) { TAMPERING_DETECTED = true; TAMPERING_DETECTIONS++; break; }" << std::endl;
					}
					INDT_5 << "}" << std::endl;
				}
			}

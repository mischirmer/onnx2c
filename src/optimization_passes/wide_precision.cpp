...
static bool is_quantized_weight_input(Node* n, Tensor* t)
{
	if (!n || !t)
		return false;
...
static bool is_used_as_quantized_weight(Tensor* t)
{
	for (auto* consumer : t->consumers) {
		if (is_quantized_weight_input(consumer, t))
			return true;
	}
	return false;
}
...
void Graph::wide_precision(void)
{
	...
	for (auto* t : tensors) {
		if (!is_promotable_int8_weight(t) || !is_used_as_quantized_weight(t))
			continue;

		const auto old_type = t->data_type;
		promote_int8_weight(t);
		LOG(DEBUG) << "Promoted quantized weight tensor " << t->name
